                        ...
                        foreach (ItemInstance slot in slotNotFull)
                        {
                            ...
                            if (ServerManager.Instance.Configuration.UseLogService)
                            {
                                LogServiceClient.Instance.LogPacket(new PacketLogEntry()
                                {
                                    ...
                                    Packet = $"[ITEM_CREATE]IIId: {slot.Id} ItemVNum: {slot.ItemVNum} Amount: {max - slot.Amount} MapId: {Owner.MapInstance?.Map.MapId} MapX: {Owner.PositionX} MapY: {Owner.PositionY}"
                                });
                            }
            ...
            if (Owner != null)
            {
                ...
                if (ServerManager.Instance.Configuration.UseLogService)
                {
                    LogServiceClient.Instance.LogPacket(new PacketLogEntry()
                    {
                        ...
                        SenderId = Owner.CharacterId,
                        PacketType = LogType.ItemCreate,
                        Packet = $"[ITEM_CREATE]IIId: {itemInstance.Id} ItemVNum: {itemInstance.ItemVNum} Amount: {itemInstance.Amount} MapId: {Owner.MapInstance?.Map.MapId} MapX: {Owner.PositionX} MapY: {Owner.PositionY}"
                    });
                }
