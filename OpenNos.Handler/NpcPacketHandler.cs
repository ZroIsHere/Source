                ...
                case BuyShopType.CharacterShop:
                    ...
                    if (BuyValidate(Session, shop, buyPacket.Slot, amount))
                    {
                        ...
                        if (ServerManager.Instance.Configuration.UseLogService)
                        {
                            ...
                            {
                                Sender = ServerManager.Instance.GetSessionByCharacterId(buyPacket.OwnerId)?.Character.Name ?? "Unknown",
                                ...
                                Packet = $"[ITEM_BUY_PLAYERSHOP]IIId: {item.ItemInstance.Id} ItemVNum: {item.ItemInstance.ItemVNum} Amount: {buyPacket.Amount} Rare: {item.ItemInstance.Rare} Upgrade: {item.ItemInstance.Upgrade} PricePer: {item.Price}"
                            });
                        }
                    }
                    else
