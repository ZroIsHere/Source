                                        ...
                                        if (inv != null)
                                        {
                                            ...
                                            if (characterDropperId > 0)
                                            {
                                                if (ServerManager.Instance.Configuration.UseLogService)
                                                {
                                                    ...
                                                    {
                                                        Sender = ServerManager.Instance.GetSessionByCharacterId(characterDropperId)?.Character?.Name ?? DAL.DAOFactory.CharacterDAO.LoadById(characterDropperId)?.Name ?? "Unknown",
                                                        ...
                                                        Packet = $"[GET_DROP]IIId: {inv.Id} ItemVNum: {inv.ItemVNum} Amount: {amount} Rare: {inv.Rare} Upgrade: {inv.Upgrade}"
                                                    });
                                                }
                                            }
